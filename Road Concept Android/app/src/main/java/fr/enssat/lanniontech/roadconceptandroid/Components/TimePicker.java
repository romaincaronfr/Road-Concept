...
public class TimePicker extends FrameLayout {
     ...
     * later in the code.
     */
    private static final TimePicker.OnTimeChangedListener NO_OP_CHANGE_LISTENER = new TimePicker.OnTimeChangedListener() {
    ...
    public TimePicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        mHourPicker.setOnScrollListener(new NumberPicker.OnScrollListener() {
            ...
            public void onScrollStateChange(NumberPicker numberPicker, int i) {
                if (i == NumberPicker.OnScrollListener.SCROLL_STATE_IDLE) {
                    ...
                    mCurrentHour = numberPicker.getValue();
                    if (!mIs24HourView) {
                        // adjust from [1-12] to [0-11] internally, with the times
                        // written "12:xx" being the start of the half-day
                        if (mCurrentHour == 12) {
        ...
        mMinutePicker.setOnScrollListener(new NumberPicker.OnScrollListener() {
            @Override
            public void onScrollStateChange(NumberPicker numberPicker, int i) {
                if (i == NumberPicker.OnScrollListener.SCROLL_STATE_IDLE) {
                    mCurrentMinute = numberPicker.getValue();
                    onTimeChanged();
                }
            }
        });
        ...
        mSecondPicker.setOnScrollListener(new NumberPicker.OnScrollListener() {
            @Override
            public void onScrollStateChange(NumberPicker numberPicker, int i) {
                mCurrentSeconds = numberPicker.getValue();
                onTimeChanged();
            }
        });
    ...
    private static class SavedState extends BaseSavedState {
        ...
        public static final Creator<TimePicker.SavedState> CREATOR
                = new Creator<TimePicker.SavedState>() {
            public TimePicker.SavedState createFromParcel(Parcel in) {
                return new TimePicker.SavedState(in);
            }

            public TimePicker.SavedState[] newArray(int size) {
                return new TimePicker.SavedState[size];
            }
        };
    }
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        return new TimePicker.SavedState(superState, mCurrentHour, mCurrentMinute);
    }

    @Override
    protected void onRestoreInstanceState(Parcelable state) {
        TimePicker.SavedState ss = (TimePicker.SavedState) state;
        super.onRestoreInstanceState(ss.getSuperState());
        setCurrentHour(ss.getHour());
        setCurrentMinute(ss.getMinute());
    }

    public void setHoursMinutesSeconds(Integer currentHour, Integer currentMinute, Integer currentSeconds){
